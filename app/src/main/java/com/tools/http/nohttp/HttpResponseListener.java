...
public class HttpResponseListener<T> implements OnResponseListener<T> {
    ...
    public HttpResponseListener(Activity activity, Request<?> request, HttpListener<T> httpCallback, boolean canCancel, boolean isLoading) {
        ...
        if (activity != null && isLoading) {
            mWaitDialog = WaitDialog.loadingDefaultDialog(activity,canCancel);;
    ...
    public void onFailed(int what, Response<T> response) {
        ...
        if (exception instanceof NetworkError) {// 网络不好
            ...
            ToastUtil.showShort(mActivity, R.string.error_system_unsupport_method);
        } else if (exception instanceof ParseError) {
            ToastUtil.showShort(mActivity, R.string.error_parse_data_error);
        } else {
