package com.abing.androidnote.androida笔记.校验;
...
public class CheckUtils {
    ...
    //将手机号设置为132****7940格式
    private String setHidePhone(String phone)
    {
        Pattern p = Pattern.compile("^(\\+86)?1[34578]\\d{9}$");
        Matcher m = p.matcher(phone);
        String s = "";
        if (m.matches())//判断是否是正确手机号，
        {
    ...
    //将身份证号设置为3708*****7940格式
    private String setHideidCard(String idCard)
    {
        Pattern p = Pattern.compile("^\\d{15}|\\d{18}|(\\d{17}(\\d|X|x))$");
        Matcher m = p.matcher(idCard);
        String s = "";
        if (m.matches())///判断是否是正确身份证号码格式，
        {
    ...
    //将银行卡号设置为6228*****7940格式
    private String setHideBankCard(String bankCard)
    {
        String s="";
        if (checkBankCard(bankCard))
        {
            s=bankCard.substring(0, 4) + "*****" + bankCard.substring(bankCard.length()-4,bankCard.length());
        }else {
            s=bankCard;
        }
        return s;
